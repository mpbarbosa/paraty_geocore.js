...
describe('Integration: browser GeolocationPosition → GeoPosition', () => {
    describe('non-enumerable coords extraction', () => {
        it('reads latitude and longitude from non-enumerable getters', () => {
            const raw = (0, fixtures_1.makeBrowserPosition)(SAO_PAULO, fixtures_1.TEST_TIMESTAMP);
        ...
        it('spread of raw coords is empty (confirming non-enumerable behaviour)', () => {
            const raw = (0, fixtures_1.makeBrowserPosition)(SAO_PAULO);
        ...
        it('GeoPosition still extracts coords correctly despite non-enumerable getters', () => {
            const raw = (0, fixtures_1.makeBrowserPosition)({ ...SAO_PAULO, altitude: 760, altitudeAccuracy: 5, heading: 90, speed: 0 });
            const pos = new GeoPosition_1.default(raw);
    ...
    describe('accuracyQuality classification', () => {
        it.each([
            ...
            const pos = new GeoPosition_1.default((0, fixtures_1.makeBrowserPosition)({ ...SAO_PAULO, accuracy }));
            expect(pos.accuracyQuality).toBe(expected);
        });
    });
    describe('immutability', () => {
        it('instance is frozen after construction from browser position', () => {
            const pos = new GeoPosition_1.default((0, fixtures_1.makeBrowserPosition)(SAO_PAULO));
    ...
    describe('distanceTo — real calculateDistance (not mocked)', () => {
        it('calculates SP→RIO distance within 1% of expected value', () => {
            const sp = new GeoPosition_1.default((0, fixtures_1.makeBrowserPosition)(SAO_PAULO));
            const rio = new GeoPosition_1.default((0, fixtures_1.makeBrowserPosition)(RIO_DE_JANEIRO));
    ...
    describe('toString — full position string', () => {
        it('produces correct format from browser position', () => {
            const raw = (0, fixtures_1.makeBrowserPosition)({ latitude: -23.5505, longitude: -46.6333, accuracy: 12, altitude: 760, altitudeAccuracy: null, heading: null, speed: 0 }, fixtures_1.TEST_TIMESTAMP);
            const pos = new GeoPosition_1.default(raw);
